...
define(["exports", "scene"], function (exports, scene) {
    ...
    function zBufferTest(x, y, z, color) {
        ...
        if (color.rgbaShaded[0] == 0 && color.rgbaShaded[1] == 0 && color.rgbaShaded[2] == 0) {
            return true;
        }
        }
                
        if (Math.abs(z - zBuf[indexZBuf]) < 0.1) {
            // color.rgbaShaded=[0,0,0,0];


            return false;
        }

        var test = zBuf[indexZBuf];
        if (z < zBuf[indexZBuf]) {
            return false;
        }
    ...
    function getRed(x, y) {
        ...
        return framebuffer[(y * width + x) * 4];
    }
    
    function getRGB(x,y)
    {
        return [framebuffer[(y * width + x) * 4],framebuffer[(y * width + x) * 4 +1],framebuffer[(y * width + x) * 4 +2]];
    }

    function isBlack(x, y) {

        return (framebuffer[(y * width + x) * 4] + framebuffer[(y * width + x) * 4 + 1] + framebuffer[(y * width + x) * 4 + 2] == 0);
    }
    //
